...
(hashlist '(("gmt" . 0) ("ut" . 0) ("utc" . 0) ("est" . 5)
            ("edt" . 4) ("cst" . 6)
            ("cdt" . 5) ("mst" . 7)
            ("mdt" . 6)	("pst" . 8)
            ("pdt" . 7)
...
(defstruct decoded-time
  ...
  (zone   nil    :type (or rational null))   ; Value between -24 and 24 inclusive.
  ...
  (let ((default-time (make-decoded-time)))
    (multiple-value-bind (sec min hour day mon year dotw dst zone)
      ...
      (if def-zone
          ...
          (setf (decoded-time-zone default-time) nil))
      (if def-dotw
...
(defun convert-to-unitime (parsed-values)
  (let ((zone (decoded-time-zone parsed-values)))
    (encode-universal-time
     (decoded-time-second parsed-values)
     ...
     (if (and zone (or (> zone 24) (< zone -24)))
...
(defun parse-time (time-string &key (start 0) (end (length time-string))
  ...
  (let* ((string-parts (decompose-string time-string :start start :end end))
    ...
    (if string-form
        (let ((parsed-values
               (make-default-time default-seconds default-minutes
